...
class WavFilePlayer(val volume: Double) {
    fun play(file: WavFile) {
        ...
        for (i in 0..<file.channelSampleCount) {
            for (j in 0..<file.channelCount) {
                val doubleSample = file.samples[j][i] * volume
    ...
    fun renderAndPlay(audioNode: AudioNode, ctx: Context, seconds: Double, channels: Int = 2) {
        ...
        thread {
            while (ctx.time < seconds) {
                val buffer = renderSegment(audioNode, 1024, ctx, channels)
    ...
    fun renderSegment(audioNode: AudioNode, bufferSize: Int, ctx: Context, channels: Int = 2): ByteArray {
        ...
        for (i in 0..<bufferSize) {
            ...
            for (channelIndex in 0..<channels) {
                ...
                val limitedSample = (sample * volume).coerceIn(-1.0, 1.0)
