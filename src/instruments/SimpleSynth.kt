...
interface Synth {
    ...
    fun buildAudio(song: Song, track: Int, random: Random): AudioNode
}
...
class SimpleSynth(
    ...
    override fun buildNode(random: Random): AudioNode {
        ...
        return SynthNode(vibratoFreq, phaseOffset, vibrato, detune, adsr, mix)
    }

    override fun buildAudio(song: Song, track: Int, random: Random): AudioNode {
        val factory = { buildNode(random) }
        val songPlayer = SongPlayer(factory, random, song, track, 36, InstrumentSettings(adsr.release, 0.0))
        return songPlayer
    }
    ...
    class SynthNode(
        ...
        private val vibratoNode = vibrato2(vibrato, vibratoFreq)
        ...
        override fun process(ctx: Context, inputs: DoubleArray): DoubleArray {
            ...
            val modFreq = vibratoNode.process(ctx, doubleArrayOf(freq))[0] * detuneAmount
